...
\begin{itemize}
\item \texttt{siamart1116.cls} (required): Main SIAM standard \LaTeX\ class file.
...
\end{itemize}
To use these files, put \texttt{siamart1116.cls} and
...
\begin{table}[tbhp]
 \captionsetup{position=top} %<- Needed for using subtables created with the subfig package 
  \caption{Example table adapted from Kolda and Mayo \rm{\cite{KoMa14}}.}
  \label{tab:KoMa14}
  \centering
  \subfloat[$\beta=1$]{
...
\documentclass[supplement]{siamart1116}
\input{ex_shared}
...
\item Change in file names: \texttt{siamltex1213.cls} is replaced by
  \texttt{siamart1116.cls}, and the \texttt{siam.bst} bibliography style
...
However, if you prefer to number objects by section, e.g., Figure 5.1
would indicate the first figure in Section 5, put the code from
